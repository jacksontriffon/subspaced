...
export function formPlural(count: number, base: string, pluralSuffix: string) {
...
export function dateToStr(date: DateOrString) {
	return dateNew(date).toLocaleString("ja-JP", { dateStyle: "short" });
}
...
export function dateElapsedToStr(date: DateOrString) {
	...
	if (hours < 1) return Math.floor(minutes) + "m";

	if (days < 1) return Math.floor(hours) + "h";

	if (months < 2) return Math.floor(days) + "d";
