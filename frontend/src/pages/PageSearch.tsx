...
export function PageSearch(props: Framework.RouteProps) {
	...
	const tokenIndex = Solid.createMemo(() => {
		const tokenStr =
			props.routeMatch()?.matches[App.Pages.Search.matchToken];
...
function SearchResults(props: {
	...
	const [limit, setLimit] = Framework.createHistorySignal(
		"limit",
		limitStart,
	);
	...
	const queryToken = Solid.createMemo(() => {
		...
		if (token.category === "prt" && token.surface_form.length === 1)
	...
	return (
		<>
					...
					{(result, index) => (
						<Solid.Switch>
							...
							<Solid.Match when={result.type === "kanji"}>
								<EntryKanji
									entry={result as App.Api.Kanji.Entry}
								/>
							</Solid.Match>
							...
							<Solid.Match
								when={
									result.type === "section" &&
									result.section === "end" &&
									(searchResults.latest?.entries[0].type !==
										"section" ||
										searchResults.latest?.entries[0]
											.section !== "sentence" ||
										searchResults.latest?.entries.length >
											3)
								}
							>
								<Framework.HorizontalBar />
			...
			<Solid.Show when={queryToken() !== ""}>
				<SearchResults query={queryToken} />
			</Solid.Show>
		</>
	);
}
