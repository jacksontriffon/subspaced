...
export async function downloadAndImport(
	...
	await File.download(logger, url, gzipFilename, useCachedFiles);

	await File.extractGzip(logger, gzipFilename, xmlFilename, useCachedFiles);
	...
	for await (const rawEntry of entryIterator) {
		try {
			...
			console.error(
				`error normalizing word entry ${rawEntry.ent_seq[0]}: ${e}`,
			);
		}
	}
			...
			sense: [
				{
					...
					gloss: [
						"Lorenzi",
						"yours truly",
						"creator of https://jisho.hlorenzi.com",
					],
				},
			],
		}),
			...
			sense: [
				{
					...
					gloss: [
						"Kenshiro",
						"green dragon character appearing in Lorenzi's logos",
					],
				},
			],
		}),
...
export function normalizeEntry(raw: JmdictRaw.Entry): Api.Word.Entry {
	...
	if (pitchEntries.length !== 0) entry.pitch = pitchEntries;
...
export function normalizeHeadings(
	...
	for (const k_ele of rawKEle ?? []) {
		for (const r_ele of rawREle) {
			...
			if (r_ele.re_nokanji !== undefined) continue;

			if (
				r_ele.re_restr !== undefined &&
	...
	for (const r_ele of rawREle) {
		...
		if (seenReadings.has(reb)) continue;
	...
	if (usuallyOnlyKana) {
		const mostCommonREle = rawREle.find(
			(r_ele) =>
				r_ele.re_inf === undefined ||
				!r_ele.re_inf.some((tag) => tag === "sk"),
		);
		...
		if (mostCommonREle) {
			const mostCommonReading = Kana.toHiragana(mostCommonREle.reb[0]);

			const mostCommonReadingNormalized = normalizeHeading(
				wordId,
				mostCommonREle,
				undefined,
				isName,
			...
			const mostCommonReadingScore = scoreHeading(
				mostCommonReadingNormalized,
			);
			...
			const firstHeadingCommonness = scoreHeading(headings[0]);

			if (
				!firstHeadingIsCorrect ||
	...
	for (let h = 0; h < headings.length; h++) {
		...
		if (
			...
			if (jlpt !== undefined) heading.jlpt = jlpt;
		}

		const score = scoreHeading(heading);
		if (score !== 0) heading.score = score;
	}
...
function normalizeHeading(
	...
	if (keb !== undefined) heading.reading = reb;
	...
	if (perfectPatch !== undefined)
		...
		const furiganaPatched = Furigana.patch(
			furiganaRevised,
	...
	if (k_ele?.ke_inf?.some((tag) => tag === "ateji")) heading.ateji = true;
	...
	if (k_ele?.ke_inf?.some((tag) => tag === "rK")) heading.rareKanji = true;
	...
	if (keb !== undefined) {
		...
		if (!onlyJouyouKanji) heading.nonJouyouKanji = true;
	}

	if (k_ele !== undefined && r_ele.re_inf?.some((tag) => tag === "gikun"))
		heading.gikun = true;

	if (r_ele.re_inf?.some((tag) => tag === "ik")) heading.irregularKana = true;

	if (r_ele.re_inf?.some((tag) => tag === "ok")) heading.outdatedKana = true;
	...
	if (!isName) {
		...
		if (rankWikipedia !== undefined) heading.rankWikipedia = rankWikipedia;
	}
	...
	for (const rankTag of k_ele?.ke_pri ?? []) {
		for (const rankField of rankFields) {
			if (rankTag.startsWith(rankField.tagPrefix)) {
				const rank = parseInt(
					rankTag.substring(rankField.tagPrefix.length),
				);
	...
	for (const rankField of rankFields) {
		...
		if (kRank !== undefined && rRank !== undefined) {
...
function scoreHeading(heading: Api.Word.Heading): number {
	let score = 0;
	...
	if (heading.rankIchi !== undefined)
		score +=
			heading.rankIchi === 1 ? 250 : heading.rankIchi === 2 ? 100 : 0;

	if (heading.rankSpec !== undefined)
		score +=
			heading.rankSpec === 1 ? 250 : heading.rankSpec === 2 ? 100 : 0;

	if (heading.rankGai !== undefined)
		score += heading.rankGai === 1 ? 50 : heading.rankGai === 2 ? 10 : 0;

	if (heading.rankAnimeDrama !== undefined)
		score += scoreCurve(heading.rankAnimeDrama, 1, 100_000, 100, 1);

	if (heading.irregularKanji) score -= 20000;

	if (heading.irregularKana) score -= 20000;

	if (heading.irregularOkurigana) score -= 20000;

	if (heading.outdatedKanji) score -= 30000;

	if (heading.outdatedKana) score -= 30000;
...
export function normalizeSenses(
	...
	for (const rawSense of rawSenses) {
		...
		for (const rawGloss of rawSense.gloss) {
			if (typeof rawGloss === "string") gloss.push(rawGloss);
			else
				gloss.push({
					text: rawGloss.text,
		...
		if (rawSense.misc) sense.misc = rawSense.misc;

		if (rawSense.field) sense.field = rawSense.field;

		if (rawSense.s_inf) sense.info = rawSense.s_inf;
		...
		if (rawSense.ant) {
			if (sense.xref === undefined) sense.xref = [];
		...
		if (rawSense.lsource) {
			...
			for (const lsource of rawSense.lsource) {
				...
				if (lsource.text !== undefined) langSrc.source = lsource.text;

				sense.lang.push(langSrc);
			}
		}

		if (rawSense.dial) sense.dialect = rawSense.dial;
		...
		if (rawSense.example) {
			...
			for (const example of rawSense.example) {
				...
				const ja = example.ex_sent.find(
					(ex) => ex.attr["xml:lang"] === "jpn",
				);
				const en = example.ex_sent.find(
					(ex) => ex.attr["xml:lang"] === "eng",
				);
...
export function normalizeXref(rawXref: string): Api.Word.CrossReference {
	const split = rawXref.split("・");

	if (split.length === 1) return { base: rawXref };

	if (split.length === 2) {
		if (Kana.hasJapanese(split[1]))
			return { base: split[0], reading: split[1] };
		else return { base: split[0], senseIndex: parseInt(split[1]) };
	}
...
function gatherPitchAccentEntries(
	...
	for (const heading of apiHeadings) {
		if (!Kana.hasKanji(heading.base)) {
			...
			const kanjiCandidates = apiHeadings.filter(
				(h) => Kana.hasKanji(h.base) && h.reading === heading.base,
			);
	...
	const pitch = pitchKeys.flatMap((p) => PitchAccent.get(p[0], p[1]));

	return [...new Set<string>(pitch)].map((p) => ({ text: p }));
}
...
export function gatherLookUpHeadings(
	...
	for (const heading of apiWord.headings) {
		...
		if (heading.reading) {
			const reading = Kana.normalizeWidthForms(
				heading.reading,
			).toLowerCase();
			add(reading, score);
...
export function gatherLookUpTags(
	...
	const partsOfSpeech = apiWord.senses.flatMap((d) => d.pos);

	const nameTag: Api.Word.FilterTag[] = partsOfSpeech.some((p) =>
		Api.Word.partOfSpeechNameTags.includes(p as any),
	)
		? ["name"]
		: [];
	...
	const glossTypes = apiWord.senses
		.flatMap((d) =>
			d.gloss.map((g) => (typeof g !== "string" ? [g.type] : [])),
		)
		.flat();

	const fieldTags = apiWord.senses.flatMap((d) => d.field ?? []);

	const miscTags = apiWord.senses.flatMap((d) => d.misc ?? []);

	const langTags = apiWord.senses
		.flatMap((d) => d.lang ?? [])
		.map((l) => l.language ?? "wasei");

	const dialectTags = apiWord.senses.flatMap((d) => d.dialect ?? []);
	...
	for (const heading of apiWord.headings) {
		if (heading.jlpt === undefined) continue;

		if (lowestJlptLevel === undefined || heading.jlpt > lowestJlptLevel)
	...
	const pitchAccentTags: Api.Word.PitchAccentTag[] = (
		apiWord.pitch ?? []
	).map((pitch) => JmdictTags.categorizePitch(pitch.text));

	return JmdictTags.expandFilterTags([
		...new Set<Api.Word.FilterTag>([
			...nameTag,
			...partsOfSpeech,
			...glossTypes,
			...fieldTags,
