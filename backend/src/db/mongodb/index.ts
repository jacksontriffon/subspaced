...
export const dbUrl = process.env.MONGO_DB_URL ?? "mongodb://localhost:27017";
...
export const fieldAnalyticsByDateValue =
	"value" satisfies keyof DbAnalyticsEntry["byDate"];
export const fieldAnalyticsByDateElements =
	"elements" satisfies keyof DbAnalyticsEntry["byDate"];
...
export const fieldWordLookUpTags =
	`${fieldWordLookUp}` + `.${"tags" satisfies keyof DbWordEntry["lookUp"]}`;

export const fieldWordLookUpChars =
	`${fieldWordLookUp}` + `.${"chars" satisfies keyof DbWordEntry["lookUp"]}`;

export const fieldKanjiLookUp = "lookUp" satisfies keyof DbKanjiEntry;

export const fieldKanjiScore = `${"score" satisfies keyof DbKanjiEntry}`;
...
export async function connect(): Promise<Interface> {
	...
	const state: State = {
		...
		collDefinitions: db.collection<DbDefinitionEntry>(
			dbCollectionDefinitions,
		),
	...
	return {
		...
		log: (text) => log(state, text),
		logGet: () => logGet(state),

		analyticsAdd: (id, amount) => analyticsAdd(state, id, amount),
		analyticsAddSet: (id, element) => analyticsAddSet(state, id, element),
		analyticsDailyGet: () => analyticsDailyGet(state),
		...
		importStandardStudylist: (studylistId, studylistName, wordIds) =>
			MongoDbStudyLists.importStandardStudylist(
				state,
				studylistId,
				studylistName,
				wordIds,
		...
		searchByInflections: (inflectionBreakdown, inflectionOf, options) =>
			MongoDbSearch.searchByInflections(
				state,
				inflectionBreakdown,
				inflectionOf,
				options,
		...
		searchByTags: (options) => MongoDbSearch.searchByTags(state, options),
		...
		listAllKanji: () => MongoDbSearch.listAllKanji(state),
		...
		streamAllWords: () => MongoDbSearch.streamAllWords(state),
		...
		studylistEditorJoin: (authUser, studylistId, password) =>
			MongoDbStudyLists.studylistEditorJoin(
				state,
				authUser,
				studylistId,
				password,
		...
		studylistEditorLeave: (authUser, studylistId) =>
			MongoDbStudyLists.studylistEditorLeave(
				state,
				authUser,
				studylistId,
			),
		...
		studylistGetAllMarked: (authUser, markWordId) =>
			MongoDbStudyLists.studylistGetAllMarked(
				state,
				authUser,
				markWordId,
			),
		studylistWordAdd: (authUser, studylistId, wordId) =>
			MongoDbStudyLists.studylistWordAdd(
				state,
				authUser,
				studylistId,
				wordId,
		...
		studylistWordRemoveMany: (authUser, studylistId, wordIds) =>
			MongoDbStudyLists.studylistWordRemoveMany(
				state,
				authUser,
				studylistId,
				wordIds,
		...
		studylistWordImport: (
			...
			MongoDbStudyLists.studylistWordImport(
				state,
				authUser,
				studylistId,
				attemptDeinflection,
		...
		studylistCommunityGetRecent: (authUser, limit) =>
			MongoDbStudyLists.studylistCommunityGetRecent(
				state,
				authUser,
				limit,
			),
...
export async function log(state: State, text: string) {
	const entry = {
		date: new Date(),
		text: text,
	};
...
export async function logGet(state: State): Promise<Api.Log.Entry[]> {
...
export async function analyticsAdd(
	...
	await state.collAnalytics.updateOne(
		...
		{
			$inc: {
				[`${fieldAnalyticsByDate}.${dateStr}.${fieldAnalyticsByDateValue}`]:
					amount,
			},
		},
		{ upsert: true },
...
export async function analyticsAddSet(
	...
	await state.collAnalytics.updateOne(
		...
		{
			$addToSet: {
				[`${fieldAnalyticsByDate}.${dateStr}.${fieldAnalyticsByDateElements}`]:
					element,
			},
		},
		{ upsert: true },
		...
		try {
			...
			for (const entry of dateEntries) {
				...
				if (Array.isArray(entry.elements)) {
					for (const elem of entry.elements) elements.add(elem);
				}
			}
...
export async function insertWithNewId<T extends MongoDb.Document>(
	...
	while (true) {
		tries++;
		if (tries > 10) throw Api.Error.internal;

		id = generateRandomId();

		if (id.startsWith("00")) continue;

		try {
			await collection.insertOne({ ...entry, _id: id });
		} catch {
			continue;
		}
...
export function translateWordDbToApi(dbWord: DbWordEntry): Api.Word.Entry {
...
export function translateKanjiDbToApi(dbKanji: DbKanjiEntry): Api.Kanji.Entry {
	// Add and remove fields via destructuring assignment
	const { _id, lookUp, ...apiKanji } = {
		...dbKanji,
		id: dbKanji._id,
	};
