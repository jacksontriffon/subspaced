...
export interface Interface {
	...
	importWordEntriesFinish: (importStartDate: Date) => Promise<void>;

	importKanjiEntries: (entries: Api.Kanji.Entry[]) => Promise<void>;

	importKanjiWordCrossRefEntries: (
		entries: Api.KanjiWordCrossRef.Entry[],
	) => Promise<void>;
	...
	searchByTags: (options: SearchOptions) => Promise<Api.Word.Entry[]>;
	...
	listAllKanji: () => Promise<Api.Kanji.Entry[]>;

	listWordsWithChars: (chars: string[]) => Promise<Api.Word.Entry[]>;

	streamAllWords: () => AsyncGenerator<Api.Word.Entry>;

	studylistCreate: (authUser: Api.MaybeUser, name: string) => Promise<string>;
...
export function createDummy(): Interface {
	return {
		...
		studylistCreate: async () => {
			throw Api.Error.forbidden;
		},
		studylistClone: async () => {
			throw Api.Error.forbidden;
		},
