...
const argDb = process.argv.some((arg) => arg === "--db-mongo")
	? "mongo"
	: process.argv.some((arg) => arg === "--db-dummy")
	? "dummy"
	: "mongo";

const argAuth = process.argv.some((arg) => arg === "--auth-lorenzi-dev")
	? "lorenzi-dev"
	: process.argv.some((arg) => arg === "--auth-lorenzi")
	? "lorenzi"
	: process.argv.some((arg) => arg === "--auth-dummy")
	? "dummy"
	: "dummy";

console.log("starting db...");
const db = argDb === "mongo" ? await DbMongo.connect() : Db.createDummy();

console.log("starting auth...");
const auth =
	argAuth === "lorenzi-dev"
		? await AuthLorenzi.create(true)
		: argAuth === "lorenzi"
		? await AuthLorenzi.create(false)
		: Auth.createDummy();

let version = "v?.?-??????";
try {
	version = child_process
		.execSync("git describe --tags --match v*")
		.toString()
		.trim()
		.replace("-", ".");
} catch (error) {
	console.log("Git version information not available.");
	console.log("Setting custom version - Anime");
	version = "vAnime";
} finally {
}
